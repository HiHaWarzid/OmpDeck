...
test("wraps plan composer submissions with the hidden OmpDeck plan marker", () => {
	const { buildComposerPromptSubmission, OMP_DECK_PLAN_MODE_MARKER } = loadComposerBehaviorModule();

	const submission = buildComposerPromptSubmission("Inspect first", "plan");

	assert.equal(submission.message, "Inspect first");
	assert.match(submission.agentMessage, new RegExp(`^${OMP_DECK_PLAN_MODE_MARKER}\\n`));
