...
const zhCN = {
  ...
  "config.authGuideDesc": "认证配置用于存储 API Key，pi 会自动从 ~/.omp/agent/auth.json 读取。",
...
"config.emptyExtensions": "暂无已安装扩展。",
  ...
  "settings.disableUpdateCheckDesc": "开启后不再自动检测 OmpDeck 和 Pi CLI 的版本更新，设置页中检测按钮也将禁用。",
  "settings.electronSandbox": "启用 Chromium 沙箱",
  "settings.electronSandboxDesc": "控制 Electron 渲染进程沙箱（与 pi Agent 无关）。默认关闭以兼容 Windows 安全软件/旧 GPU 驱动；开启后需重启 OmpDeck。若启动异常请改回关闭。",
...
const enUS: Record<TranslationKey, string> = {
  ...
  "config.authGuideDesc": "Auth config stores API keys. pi loads them automatically from ~/.omp/agent/auth.json.",
...
"config.emptyExtensions": "No installed extensions.",
  ...
  "settings.disableUpdateCheckDesc": "When enabled, stops auto-checking updates for OmpDeck and Pi CLI. Update buttons in settings will also be disabled.",
  "settings.electronSandbox": "Enable Chromium sandbox",
  "settings.electronSandboxDesc": "Controls Electron renderer sandbox (unrelated to pi Agent). Off by default for Windows AV/GPU compatibility. Requires restarting OmpDeck. Turn it back off if the app fails to start.",
  ...
  "settings.restartNotice":
    "Title bar changes take effect after restarting OmpDeck.",
