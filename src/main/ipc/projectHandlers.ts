...
export function registerProjectHandlers(deps: ProjectHandlerDeps) {
	...
	ipcMain.handle(ipcChannels.projectsReorder, async (_event, projectIds: string[]) => {
	...
	ipcMain.handle(
		ipcChannels.projectResourcesCreateSkill,
		async (_event, input: CreateProjectSkillInput) => {
	...
	ipcMain.handle(
		ipcChannels.projectResourcesDeleteSkill,
		async (_event, projectId: string, skillPath: string) => {
	...
	ipcMain.handle(
		ipcChannels.projectResourcesDeleteExtension,
		async (_event, projectId: string, extensionPath: string) => {
	...
	ipcMain.handle(
		ipcChannels.projectResourcesToggleSkill,
		async (_event, projectId: string, skillPath: string, enabled: boolean) => {
	...
	ipcMain.handle(
		ipcChannels.projectResourcesToggleExtension,
		async (_event, projectId: string, extensionPath: string, enabled: boolean) => {
	...
	ipcMain.handle(
		ipcChannels.projectResourcesRenameSkill,
		async (_event, projectId: string, skillPath: string, newName: string) => {
	...
	ipcMain.handle(ipcChannels.projectsListWorktreeChildren, async (_event, parentId: string) => {
	...
	ipcMain.handle(ipcChannels.projectsSetChatPath, async (_event, path: string) => {
