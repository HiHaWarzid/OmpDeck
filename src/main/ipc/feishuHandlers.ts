...
import { ipcTable, ipcChannels, type IpcHandlerMap } from "../../shared/ipc";
...
type FeishuHandlerMaps = {
	feishu: IpcHandlerMap<typeof ipcTable.feishu, PiDesktopApi["feishu"]>;
};

export function registerFeishuHandlers(deps: FeishuHandlerDeps): FeishuHandlerMaps {
	...
	return {
		feishu: {
			...
			// 添加 Bot
			botAdd: async (_event, input: FeishuConnectInput) => {
			...
			// 删除 Bot
			botRemove: async (_event, botId: string) => {
				const existing = getFeishuBridge();
				if (existing) {
					existing.stop();
					setFeishuBridge(null);
			...
			// 更新 Bot 配置
			botConfig: async (_event, botId: string, patch: Partial<FeishuBotConfig>) => {
			...
			botSecret: async (_event, botId: string) => {
				return getDecryptedBotAppSecret(botId);
			},

			// 测试连接
			testConnection: async (_event, appId: string, appSecret: string) => {
				// 创建临时 bridge 实例来测试连接
			...
			// 移除绑定
			bindingRemove: async (_event, chatId: string) => {
				const bridge = getFeishuBridge();
				if (bridge) {
			...
			// 更新绑定
			bindingUpdate: async (_event, chatId: string, patch: Partial<FeishuChatBinding>) => {
				const bridge = getFeishuBridge();
				if (bridge) {
			...
			sessionBotSet: async (_event, agentId: string, botId: string | null) => {
				const bridge = getFeishuBridge();
				if (!botId) {
