...
import type { CreateAgentInput, PiCommand, SendPromptInput, SendPromptResult } from "../../shared/types";
...
type AgentHandlerMaps = {
	agents: Pick<IpcHandlerMap<typeof ipcTable.agents, PiDesktopApi["agents"]>,
		| "list" | "getMessages" | "create" | "rename" | "stop" | "prompt" | "abort" | "exportHtml"
...
export function registerAgentHandlers(deps: AgentHandlerDeps): AgentHandlerMaps {
	...
	return {
		agents: {
			notifyAsk: async (_event, title: unknown, body: unknown) => {
			...
			getMessages: async (_event, agentId: string) =>
				agentManager.getMessages(agentId),
			create: async (_event, input: CreateAgentInput) => {
			...
			prompt: async (_event, input: SendPromptInput) => {
				...
				if (bridgeConnected && hasFeishuBinding) {
					...
					if (filePath) {
						...
						// 消息已由主进程直接处理；历史行为返回 undefined（桌面契约声明 SendPromptResult）
						return undefined as unknown as SendPromptResult;
					}
				}
				...
				// 飞书文件直发分支会 return undefined（历史行为）；桌面契约声明 SendPromptResult
				return result as SendPromptResult;
			},
			abort: async (_event, agentId: string) => {
			...
			exportHtml: async (_event, agentId: string) =>
				// pi RPC export_html 返回 { path }（data 未在 AgentManager 侧定型，此处按契约收敛）
				agentManager.exportHtml(agentId) as Promise<{ path: string }>,
			getForkMessages: async (_event, agentId: string) =>
				agentManager.getForkMessages(agentId),
			forkSession: async (_event, agentId: string, entryId: string) =>
				// pi RPC fork 返回 { text?, cancelled? }（data 未定型，此处按契约收敛）
				agentManager.forkSession(agentId, entryId) as Promise<{ text?: string; cancelled?: boolean }>,
			cloneSession: async (_event, agentId: string) => {
				// pi RPC clone 返回 { cancelled? }（data 未定型，此处按契约收敛）
				const result = await agentManager.cloneSession(agentId) as { cancelled?: boolean };
				void appLogger.info("agent", "Agent session cloned", { agentId });
				return result;
			},
			switchSession: async (_event, agentId: string, sessionPath: string) => {
				// pi RPC switch 返回 { cancelled? }（data 未定型，此处按契约收敛）
				const result = await agentManager.switchSession(agentId, sessionPath) as { cancelled?: boolean };
				void appLogger.info("agent", "Agent switched session", { agentId, sessionPath });
				return result;
			},
			editMessage: async (_event, agentId: string, messageId: string, text: string) => {
				await agentManager.editMessage(agentId, messageId, text);
				void appLogger.info("agent", "Message edited", { agentId, messageId });
			},
			deleteMessage: async (_event, agentId: string, messageId: string) => {
				await agentManager.deleteMessage(agentId, messageId);
				void appLogger.info("agent", "Message deleted", { agentId, messageId });
			},
			prepareResend: async (_event, agentId: string, messageId: string) => {
			...
			runtimeState: async (_event, agentId: string) =>
				agentManager.getRuntimeState(agentId),
			cycleModel: async (_event, agentId: string) =>
				agentManager.cycleModel(agentId),
			availableModels: async (_event, agentId: string) =>
				agentManager.getAvailableModels(agentId),
			setModel: async (_event, agentId: string, provider: string, modelId: string) => {
			...
			commands: async (_event, agentId: string) => {
				try {
					// getCommands 返回未定型数组，桌面契约声明 PiCommand[]
					return (await agentManager.getCommands(agentId)) as PiCommand[];
				} catch {
